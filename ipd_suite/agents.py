...
class QLearningAgent(Agent):
    ...
    def for_game_length(cls, name: str, expected_game_length: int):
        ...
        elif expected_game_length <= 25:
            # Long games: gradual learning, long horizon, minimal exploration
            return cls(name, alpha=0.1, gamma=0.9, epsilon=0.1)
        else:
            # Very long games (50+): very gradual learning, very long horizon, very minimal exploration
            # Lower alpha for stability, high gamma for long-term planning, very low epsilon for exploitation
            return cls(name, alpha=0.05, gamma=0.95, epsilon=0.05)
        
    def _get_state(self, own_history: List[str], opponent_history: List[str]) -> Tuple:
        """Define state based on last moves and game context for longer games"""
        if not own_history:
            return ('START', 'START', 'EARLY')
        
        # For longer games, include more context in state representation
        last_own = own_history[-1]
        last_opp = opponent_history[-1]
        
        # Add game phase information for better long-term strategy
        game_length = len(own_history)
        if game_length <= 5:
            phase = 'EARLY'
        elif game_length <= 15:
            phase = 'MID'
        ...
        # For very long games, also consider opponent's recent pattern
        if game_length >= 3:
            recent_pattern = ''.join(opponent_history[-3:])
            # Categorize common patterns
            if recent_pattern == 'CCC':
                pattern = 'COOP'
            elif recent_pattern == 'DDD':
                pattern = 'DEFECT'
            elif 'CD' in recent_pattern or 'DC' in recent_pattern:
                pattern = 'MIXED'
            else:
                pattern = 'OTHER'
        else:
        ...
        return (last_own, last_opp, phase, pattern)
...
class ThompsonSampling(Agent):
    ...
    def __init__(self, name: str, base_learning_rate: float = 0.2, learning_rate_scale: float = 1.2):
        ...
        # For longer games, track recent performance separately
        self.recent_window = 10  # Track recent performance window
        self.recent_coop_rewards = []
        self.recent_defect_rewards = []
    ...
    def for_game_length(cls, name: str, expected_game_length: float):
        ...
        elif expected_game_length <= 30:
            # Long games: gradual learning, conservative sensitivity
            return cls(name, base_learning_rate=0.05, learning_rate_scale=0.7)
        else:
            # Very long games (50+): very gradual learning, very conservative sensitivity
            # Lower learning rates for stability in long interactions
            return cls(name, base_learning_rate=0.02, learning_rate_scale=0.5)
    ...
    def update(self, own_move: str, opponent_move: str, payoff: float):
        ...
        # Track recent rewards for better long-term adaptation
        if own_move == 'C':
            self.recent_coop_rewards.append(payoff)
            if len(self.recent_coop_rewards) > self.recent_window:
                self.recent_coop_rewards.pop(0)
        else:
            self.recent_defect_rewards.append(payoff)
            if len(self.recent_defect_rewards) > self.recent_window:
                self.recent_defect_rewards.pop(0)
        
        # Calculate adaptive learning rate based on payoff and game length parameters
        # For longer games, also consider recent performance trends
        base_learning_rate = self.base_learning_rate
        
        # In longer games, adjust learning rate based on recent performance stability
        if len(self.recent_coop_rewards) >= 5 or len(self.recent_defect_rewards) >= 5:
            # If recent performance is consistent, reduce learning rate for stability
            if own_move == 'C' and len(self.recent_coop_rewards) >= 3:
                recent_var = np.var(self.recent_coop_rewards[-5:]) if len(self.recent_coop_rewards) >= 5 else 0
                if recent_var < 1.0:  # Low variance means stable performance
                    base_learning_rate *= 0.8
            elif own_move == 'D' and len(self.recent_defect_rewards) >= 3:
                recent_var = np.var(self.recent_defect_rewards[-5:]) if len(self.recent_defect_rewards) >= 5 else 0
                if recent_var < 1.0:
                    base_learning_rate *= 0.8
        
        learning_rate = base_learning_rate + (payoff / 5.0) * self.learning_rate_scale
        ...
        if own_move == 'C':
            ...
            if payoff < 2.5:  # Below average
                penalty = (2.5 - payoff) * base_learning_rate
                self.beta_c += penalty
        else:
...
class GradientMetaLearner(Agent):
    ...
    def __init__(self, name: str, learning_rate: float = 0.05, expected_game_length: int = 10):
        super().__init__(name)
        self.learning_rate = learning_rate
        self.expected_game_length = expected_game_length
        
        # Expand feature set for longer games
        n_features = 8 if expected_game_length > 25 else 5
        self.weights = np.zeros(n_features)  # Feature weights
        self.feature_history = []
        self.action_history = []
        self.reward_history = []
        
        # For longer games, add momentum to weight updates
        self.momentum = 0.9 if expected_game_length > 25 else 0.0
        self.velocity = np.zeros(n_features)
    
    @classmethod
    def for_game_length(cls, name: str, expected_game_length: int):
        """Factory method to create GradientMetaLearner with optimal parameters for game length"""
        if expected_game_length <= 5:
            # Short games: higher learning rate for quick adaptation
            return cls(name, learning_rate=0.1, expected_game_length=expected_game_length)
        elif expected_game_length <= 15:
            # Medium games: moderate learning rate
            return cls(name, learning_rate=0.05, expected_game_length=expected_game_length)
        elif expected_game_length <= 35:
            # Long games: lower learning rate for stability
            return cls(name, learning_rate=0.02, expected_game_length=expected_game_length)
        else:
            # Very long games: very low learning rate with momentum
            return cls(name, learning_rate=0.01, expected_game_length=expected_game_length)
        
    def _extract_features(self, own_history: List[str], opponent_history: List[str]) -> np.ndarray:
        """Extract features from game state - expanded for longer games"""
        ...
        features[1] = opponent_history.count('C') / game_length
        ...
        # Feature 4: Game progress (normalized for expected game length)
        features[4] = min(game_length / self.expected_game_length, 1.0)
        
        # Additional features for longer games
        if n_features > 5:
            # Feature 5: Opponent defection streaks (longest streak)
            max_defect_streak = 0
            current_streak = 0
            for move in opponent_history:
                if move == 'D':
                    ...
                    current_streak = 0
            features[5] = min(max_defect_streak / 5.0, 1.0)  # Normalize
            
            # Feature 6: Alternating pattern detection
            if game_length >= 4:
                alternating_count = 0
                for i in range(1, game_length):
                    if opponent_history[i] != opponent_history[i-1]:
                        alternating_count += 1
                features[6] = alternating_count / (game_length - 1)
            
            # Feature 7: Long-term trend (cooperation trend over windows)
            if game_length >= 10:
                window_size = min(10, game_length // 3)
                early_coop_rate = opponent_history[:window_size].count('C') / window_size
                late_coop_rate = opponent_history[-window_size:].count('C') / window_size
                features[7] = late_coop_rate - early_coop_rate  # Trend: -1 to 1
    ...
    def update_policy(self):
        """Update weights using policy gradient with momentum for longer games"""
        ...
        for i in range(len(self.feature_history)):
            ...
            total_grad += advantage * grad
        
        # Normalize by number of samples
        if len(self.feature_history) > 0:
            total_grad /= len(self.feature_history)
        
        # Apply momentum for longer games (helps with stability)
        if self.momentum > 0:
            self.velocity = self.momentum * self.velocity + self.learning_rate * total_grad
...
class ClaudeCodeAgent(LLMAgent):
    ...
    def __init__(self, name: str, model: str = "claude-code", temperature: float = 0.7,
        ...
        # No api_key parameter needed - Claude Code SDK handles authentication locally
        super().__init__(name, model, temperature, termination_prob, match_history=match_history)

        # Defer claude-code-sdk import to avoid blocking during initialization
        self.claude_query = None
        self._claude_imported = False

    def _call_api(self, prompt: str) -> str:
        """Call Claude Code SDK instead of external API"""
        import asyncio

        # Import claude-code-sdk on first use to avoid blocking during initialization
        ...
        try:
            ...
            # Estimate tokens (since Claude Code SDK might not provide exact counts)
            estimated_input = len(prompt.split()) * 1.3
            ...
            if "command failed with exit code 1" in error_msg or "fatal error in message reader" in error_msg:
                ...
                raise Exception(
                    f"Claude Code SDK connection timeout. "
                    f"Please check that Claude Code is running and responsive. "
                    f"Original error: {e}"
                )
            else:
    ...
    async def _run_async_query(self, prompt: str) -> str:
        """Run the async Claude Code SDK query"""
        response_parts = []
    ...
    def _call_api_with_caching(self, static_content: str, dynamic_content: str) -> str:
        """For compatibility - Claude Code SDK doesn't need separate caching calls"""
