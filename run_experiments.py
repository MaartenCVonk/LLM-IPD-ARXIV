...
from ipd_suite import (
    ...
    Tournament, MatchHistoryManager,
...
def reconstruct_initial_population_from_config(experiment_dir: str, api_keys: dict, 
    ...
    try:
        ...
        initial_agents = create_agents(api_keys, config_temp_settings, shadow, include_classical=True, use_claude_code=use_claude_code)
...
def find_best_resume_point_with_csv_fallback(experiment_dir: str, shadow: float,
                                            ...
                                            temperature_settings: dict, use_claude_code: bool = False):
    """
    ...
    initial_agents = create_agents(api_keys, temperature_settings, shadow, include_classical=True, use_claude_code=use_claude_code)
...
def create_agents(api_keys: Dict[str, str],
                 ...
                 history_manager: MatchHistoryManager = None,
                 use_claude_code: bool = False) -> List:
    ...
    if include_classical:
        agents.extend([
            ...
            # 14-16: Adaptive learning strategies (optimized for long games)
            # Calculate expected game length: 1 / termination_prob
            QLearningAgent.for_game_length("QLearning", int(1 / termination_prob)),                   # 14
            ThompsonSampling.for_game_length("ThompsonSampling", 1 / termination_prob),          # 15
            GradientMetaLearner.for_game_length("GradientMetaLearner", int(1 / termination_prob))     # 16
        ])
...
def run_main_experiments(shadow_conditions: List[float] = [0.1, 0.25, 0.75],
    ...
    for i, shadow in enumerate(shadow_conditions):
        ...
        if evolutionary:
            ...
            initial_agents = create_agents(api_keys, temperature_settings, shadow, include_classical=True, history_manager=history_manager, use_claude_code=use_claude_code)
            ...
            if resume_experiment:
                ...
                start_phase, current_population, population_history, source_info = find_best_resume_point_with_csv_fallback(
                    experiment_dir, shadow, n_phases, api_keys, temperature_settings, use_claude_code
                )
                        ...
                        for instance in range(count):
                            ...
                            if any(x in agent_name for x in ['GPT4', 'GPT5', 'Claude', 'Mistral', 'Gemini']):
                                ...
                                if original_agent.__class__.__name__ == 'ClaudeCodeAgent':
                                    ...
                                    # For regular LLM agents, determine which API key to use
            ...
            agents = create_agents(api_keys, temperature_settings, shadow,
                                 include_classical=True, history_manager=history_manager, use_claude_code=use_claude_code)
