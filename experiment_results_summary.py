...
def load_analysis_data() -> dict:
    ...
    with open('/mnt/c/Github/LLM-IPD-ARXIV2/results/all_experiments_composition_analysis.json', 'r') as f:
        return json.load(f)
...
def get_strategy_category(strategy: str) -> str:
    ...
    if 'Claude' in strategy:
        ...
        return 'Gemini'
    elif any(llm in strategy for llm in ['GPT', 'o3']):
        return 'OpenAI'
...
def summarize_experiment_results():
    ...
    for shadow_key in sorted(experiments_by_shadow.keys()):
        ...
        output_file = f'/mnt/c/Github/LLM-IPD-ARXIV2/experiment_results_{shadow_key.replace(" ", "_").replace("%", "")}.csv'
